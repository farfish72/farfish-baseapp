...
export default function Header({ title }: { title: string }) {
  ...
  return (
    <div className="w-full px-4 pt-6 pb-8 animate-slide-up">
      {/* Top section with app name - left aligned like premium apps */}
      <div className="flex items-start justify-start mb-6">
        <div className="flex flex-col">
          <h1 className="text-3xl font-display font-bold text-premium-xl gradient-text-premium mb-1">
            FarFISH
          </h1>
          <div className="flex items-center gap-3 mt-1.5">
            <div className="relative">
              <div className="w-3 h-3 bg-green-400 rounded-full animate-pulse shadow-soft"></div>
              <div className="absolute inset-0 w-3 h-3 bg-green-400 rounded-full animate-ping opacity-60"></div>
              <div className="absolute inset-0 w-3 h-3 bg-green-300 rounded-full animate-pulse opacity-40" style={{ animationDelay: '0.5s' }}></div>
            </div>
      ...
      <div className="mb-8">
        <div className="glass-card-hover rounded-3xl p-5 border border-white/15 shadow-glass group">
          <div className="absolute inset-0 bg-premium-gradient opacity-50 rounded-3xl"></div>
          <div className="relative z-10">
            <p className="text-base font-semibold text-premium text-center leading-relaxed mb-3">
              Daily on-chain habits for future rewards on Base
            </p>
            <div className="flex justify-center">
              <div className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 bg-primary-400 rounded-full animate-pulse shadow-glow"></div>
                <div className="w-1.5 h-1.5 bg-accent-400 rounded-full animate-pulse shadow-glow-accent" style={{ animationDelay: '0.7s' }}></div>
                <div className="w-1.5 h-1.5 bg-blue-400 rounded-full animate-pulse shadow-soft" style={{ animationDelay: '1.4s' }}></div>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center gap-5">
        <div className={`relative w-16 h-16 rounded-4xl bg-gradient-to-br ${gradient} flex items-center justify-center ${shadow} transition-all duration-300 hover:scale-110 animate-bounce-gentle shadow-elevated group`}>
          <div className="absolute inset-0 rounded-4xl bg-gradient-to-br from-white/25 to-white/5"></div>
          <div className="absolute inset-0 rounded-4xl bg-shimmer opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
          <div className="text-white relative z-10 filter drop-shadow-lg">
            {icon}
          </div>
        </div>
        <div className="flex-1">
          <h2 className={`text-4xl font-display font-bold text-premium-xl bg-gradient-to-r ${gradient} bg-clip-text text-transparent mb-2 leading-tight`}>
            {title}
          </h2>
      ...
      {/* Enhanced divider */}
      <div className="mt-8 h-px bg-gradient-to-r from-transparent via-white/20 to-transparent shadow-soft"></div>
    </div>
  );
}
